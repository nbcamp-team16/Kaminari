...
let shared = MapManager.shared

class SearchResultViewController: UIViewController {
    
    var weather: Weather?
    var locationManager = MapManager.locationManager
    var latitude: Double = MapManager.shared.newLatitude
    var longtitude: Double = MapManager.shared.newLongitude
...
extension SearchResultViewController {
    func setupBarButtonItem() {
        let cancelBtn: UIBarButtonItem = UIBarButtonItem(title: "취소", style: .plain, target: self, action: #selector(cancel))
        let addBtn: UIBarButtonItem = UIBarButtonItem(title: "추가", style: .plain, target: self, action: #selector(add))
    ...
    @objc func add() {
        print("추가")
        locationData.append(tempSave!)
        print(locationData)
        SearchViewController().fetchData(locationArray: locationData)
        self.navigationController?.dismiss(animated: true)    }
}
